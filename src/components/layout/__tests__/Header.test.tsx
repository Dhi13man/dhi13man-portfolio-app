...
describe('Header', () => {
  ...
  describe('Header_whenRendered_thenDisplaysNavigationLinks', () => {
    it('Header_whenRendered_thenDisplaysEveryCanonicalNavigationLink', () => {
      // Arrange
      const expectedLinks = [
        { name: 'About', href: '/' },
  ...
  describe('Header_whenPathMatches_thenShowsActiveState', () => {
    it.each([
      ...
      ['/education/', 'Education'],
    ])('Header_whenPathIs%s_thenMarks%sAsActive', (path, linkName) => {
      // Arrange
  ...
  describe('Header_whenMobileMenu_thenTogglesCorrectly', () => {
    ...
    it('should close mobile menu when a link is clicked', async () => {
      ...
      const mobileMenu = screen
        ...
        .closest('nav')
      const mobileLinks =
        mobileMenu?.parentElement?.querySelectorAll('#mobile-menu a')
    ...
    it('should have correct aria-controls for mobile menu', async () => {
      ...
      expect(
        screen.getByRole('button', { name: /close menu/i }),
      ).toHaveAttribute('aria-controls', 'mobile-menu')
    })
  })
